...
train_config=conf/train_unified_conformer_pinyin.yaml
cmvn=true
dir=exp/train_unified_conformer
checkpoint=
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for x in dev test ${train_set}; do
        python tools/prepare_wenet_format_data.py -d ${x}
    done
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    # Nvidia 3090 显卡，目前Pytorch 1.7.1使用nccl训练有问题
    dist_backend="gloo"
    cmvn_opts=
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for mode in ctc_greedy_search ctc_prefix_beam_search attention attention_rescoring; do
    {
        ...
        mkdir -p $test_dir
        python wenet/bin/recognize_pinyin.py --gpu 0 \
            --mode $mode \
            --config $dir/train.yaml \
            --test_data $feat_dir/valid/format.data \
         ...
         # --char=1 时会对识别文本的行进行按字符各种处理导致问题，这里改为--char=0这样对拼音就正常了
         python tools/compute-wer.py --char=0 --v=1 \
            $feat_dir/valid/text $test_dir/text > $test_dir/wer
    } &
    done
    wait
fi
